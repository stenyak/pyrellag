...
def show_gallery(path):
    ...
    return render_template("gallery.html", debug=cfg()["debug_mode_enabled"], path = g.path.decode("utf-8"), parents = g.get_parents(), basename = os.path.basename(g.path.decode("utf-8")), galleries = galleries, files = g.get_files(), user = user)
